...
class 障害児福祉手当(Variable):
    ...
    def formula(対象世帯, 対象期間, parameters):
        ...
        # 障害児福祉手当は対象障害者が複数人いても同額の手当額と考えられる。
        # 参考：京都市のHPには特別児童扶養手当は手当額が「子ども1人につき」と記載されているが、
        # 　　　障害児福祉手当はその記載がない。
        # 　　　https://www.city.kyoto.lg.jp/hagukumi/page/0000254466.html
        対象障害者がいる = np.any(対象障害者である)
        手当金額 = 障害児福祉手当.金額 * 対象障害者がいる

        print(f'身体障害者手帳等級一覧: {身体障害者手帳等級一覧}')
        print(f'対象障害者手帳等級: {対象障害者手帳等級}')
        print(f'対象障害者がいる: {対象障害者がいる}')
